defmodule PaasaaTest do
  ...
  describe "Paasaa.detect" do
    ...
    test "detect" do
      assert Paasaa.detect("") == "und"
      assert Paasaa.detect("Alle menslike wesens word vry") == "afr"
      assert Paasaa.detect("এটি একটি ভাষা একক IBM স্ক্রিপ্ট") == "ben"
      assert Paasaa.detect("Alle mennesker er født frie og") == "nno"
    end
  ...
  describe "Paasaa.all" do
    test "should return a list containing language--probability tuples" do
      ...
      assert is_tuple(Enum.at(result, 0))
  ...
  describe "algorithm" do
    ...
    Enum.each @support, fn({language, index}) ->
      ...
      test "should classify #{language["name"]} (#{language["udhr"]})" do
        ...
        result = Paasaa.all(input) |> Enum.take(3)
        lang_name = Enum.at(result, 0) |> elem(0)
